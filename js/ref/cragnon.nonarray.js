...
(function (window, undefined) {
	...
	/**  
	 * Inherits from Array
	 * Need to figure out which Array Methods need to be overridden because they return a new Array instead of "this";
	 */
	cragLib.prototype = [];
	 ...
	 *  
	 */
	cragLib.prototype.init = function (selector, context, options) {
	...
	cragLib.init = cragLib.prototype.init;

	/** 
	 *  
	 */
	// [SIP]
	cragLib.prototype.makeCragnon = function (selector, context, options) {
		var self = this,
			elements = [];
	...
	cragLib.prototype.find = function (selector, options) {
	...
	cragLib.prototype.findAll = function (selector, options) {
	...
	cragLib.prototype.findFirst = function (selector, options) {};
	...
	cragLib.prototype.selectAll = function (selector, context) {
		if (!context) {
			context = doc;
		}
	...
	*/
	...
	cragLib.prototype.nodeListToArray = function (nodeList) {
		var i = 0, 
			newArray = [];
	 ...
	 *  
	 */
	cragLib.prototype.removeDuplicateNodes = function (elementArray) {
		/* TEST */
		var i = 0,
			k = 0,
	 ...
	 */
	//[OK] clone object
	cragLib.prototype.clone = function (obj) {
		var copy,
			prop,
			value,
			i;
	...
	cragLib.prototype.isNumber = function (value) { 
	...
	cragLib.prototype.isObject = function (obj) { 
	...
	cragLib.prototype.isArray = function (value) { 
		// "Javascript: The Good Parts", 'Confusion', page 61, by Douglas Crockford
	...
	cragLib.prototype.isNodeList = function (nodes) {
	...
	cragLib.prototype.isCragnon = function (obj) {
	...
	cragLib.prototype.selectorFromElement = function (element, options) {
		var selector = '', 
			id = '', 
			classes = '',
			attributes = '',
	...
	cragLib.prototype.styles = function (properties) {
		...
		if (__.isObject(properties)) {
			...
			for (arrayIndex = 0; arrayIndex < properties.length; arrayIndex++) {
				if (typeof properties[arrayIndex] === 'string') {
					...
					value = self.style(property, undefined, {"returnValue": "object"});
					values.push(value);
				}
	...
	cragLib.prototype.style = function (property, value, options) {
		var self = this,
			el = {},
			elementIndex = 0,
			currentValue = '',
			...
			for (elementIndex = 0; elementIndex < self.elements.length; elementIndex++) {
				...
				if (el.nodeType) { // is an element?
					if (typeof value === 'undefined' || value === null) { // get/return value
						currentValue = document.defaultView.getComputedStyle(el, null).getPropertyValue(property);
						if (typeof options !== 'undefined' && typeof options.returnValue !== 'undefined') { 
							console.log('options.returnValue: ' + options.returnValue);
							values.push(currentValue);
						} else {
							values.push(JSON.parse('{"' + property + '": "' + currentValue + '"}'));
						}
					} // set value instead
					else {
	...
	cragLib.selectAll = cragLib.prototype.selectAll;
	
	// END unitialized functions
